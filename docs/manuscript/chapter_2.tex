...
layers which transmit piece-wise discrete signals. Since they come into
importance in later aspects of this thesis, we use this section to provide a
short background of them.

One of the main challenges in training such activation quantized neural networks
...
\begin{definition}[Weighted finite-state automaton; \citealt{peng2018rational}]
  ...
  \begin{itemize}
    ...
    \item[--] a finite input alphabet $\Sigma$;
    \item[--] a finite state set $\mathcal{Q}$;
    \item[--] transition matrix $\Gamma: \mathcal{Q} \times \mathcal{Q} \times
...
In regards to the WFAs used in SoPa, \citet{schwartz2018sopa}
only utilize the max-sum and max-product semirings. Next, they employ constraints on the
...
Figure \ref{fig:fa} showing a representative FA without weights extracted from
...
the original runtime in Remark \ref{rmk:old_runtime}. Finally, \citet[Page 3,
...
Given the linear-chain WFA structure, each WFA in \citet{schwartz2018sopa} has a
sequence of $|\mathcal{Q}|$ states with each state $i$ having three possible
outgoing transitions; namely a self-loop transition which consumes a token but stays in the
same state $i$, a $\epsilon$-transition which does not consume a token but
transitions to state $i+1$ and a main transition which consumes a token and
transitions to state $i+1$.

To more concretely express these transitions, \citet{schwartz2018sopa} provide
the following formulation of the transition matrix $\Gamma$ under the
linear-chain structure; where $\Gamma(x)$ represents a $|Q|\times|Q|$ matrix
which represented the transition scores for an input token $x$.
$[\Gamma(x)]_{i,j}$ represents the cell value in $\Gamma(x)$ for row $i$ and
column $j$:
...
\begin{equation}
  ...
  [\Gamma(x)]_{i,j} =
  \begin{cases}
    \pmb{u}_i \cdot \pmb{v}_x + a_i  & \text{if } j = i \text{ (self-loop),} \\
    \pmb{w}_i \cdot \pmb{v}_x + b_i  & \text{if } j = i + 1 \text{ (main-transition),} \\
    \bar{0} & \text{otherwise.}
  \end{cases}
\end{equation}

Here, $\pmb{u}_i$ and $\pmb{w}_i$ are vectors and $a_i$ and $b_i$ are scalar
biases parameterizing transitions out of state $i$. $\pmb{v}_x$ represents the
word embedding for token $x$ and $\bar{0}$ represents the zero value in the
semiring used as per Definition \ref{def:semiring}. Equation \ref{eq:sopa_transition_matrix_main}
represents transitions which consume a token. Similarly, $\epsilon$-transitions
are parameterized with the following equation:
...
Correspondingly, $c_i$ represents a scalar bias provided for
$\epsilon$-transitions which are only dependent on the current state. Finally,
\citet{schwartz2018sopa} fix the initial vector $\lambda = [\bar{1}, \bar{0}, \ldots, \bar{0}]$
and the final vector $\rho = [\bar{0}, \bar{0}, \ldots, \bar{1}]$. This is a
formal notation to imply that there only exists one start and one final state
and these are present at both extremes of the linear-chain WFA. Here,
$\bar{0}$ and $\bar{1}$ represent the zero and one values from the relevant
semiring used as per Definition \ref{def:semiring}. 
