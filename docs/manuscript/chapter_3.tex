...
\begin{algorithm}[t!]
  ...
  \begin{algorithmic}[1]
    ...
    \big(\bm{h}_{i-1}[-1]\big)$ \Comment{Aggregate all hidden vector final states$^{\ddagger$}}
    \End
  \end{algorithmic}
  \algcomment{$^{\dagger}\bm{h}_{i-1}[:-1]$ follows the Python indexing syntax
    and implies keeping all elements of $\bm{h}_{i-1}$ except the last \\
    $^{\ddagger}\bm{h}_{i-1}[-1]$ follows the Python indexing syntax and implies
    retrieving the last element of the vector $\bm{h}_{i-1}$
  }
\end{algorithm}
