...
\begin{algorithm}[t!]
  ...
  \begin{algorithmic}[1]
    ...
    \State $\bm{h}_{i} \gets [\bar{1}] \mathbin\Vert \max(\bm{m'}, \bm{w'})$
    \Comment{Concatenate $\bar{1}$ to maximum of $\bm{m}$ and $\bm{\omega}$}
    \State $\pi_i \gets \text{trace}(h_i)$
    \Comment{Back-trace path $\pi_i$ corresponding to $h_i$}
    \EndFor
...
% Points:
% Mention Viterbi and max-semirings allow us to find maximum scoring paths
% Which correspond to substrings in y as well as strict linear-chain NFAs
% These can be converted into regular expressions, so each WFA-omega can have
% regular expressions extracted for each training sample
% These can be pooled together and filtered via the TauSTE layer to provide only
