...
\begin{algorithm}[t!]
  ...
  \begin{algorithmic}[1]
    ...
    \Statex
    \Begin
    \State $\bm{h}_0 \gets \big[\bar{1}, \bar{0}, \ldots, \bar{0}\big]$ \Comment{Create
    ...
    \State $\bm{h}_{i} \gets [\bar{1}] \mathbin\Vert \oplus(\bm{m'}, \bm{w'})$
    \Comment{Concatenate $\bar{1}$ to aggregate vector of $\bm{m}$ and $\bm{\omega}$}
    \EndFor
    \State \Return $\bigoplus_{i \in 1,2,...,|\bm{y}|}
    \big(\bm{h}_{i}[-1]\big)$
    \Comment{Aggregate all hidden vector final states$^{\ddagger}$}
    \End
  \end{algorithmic}
  \algcomment{
    $^*$Symbols such as $\oplus$, $\otimes$, $\bar{0}$ and $\bar{1}$ are all
    derived from max-based semirings \\
...
\ref{algo:lc_wfa_w_string_score} to compute the document score
...
utterance; as prescribed by Algorithm \ref{algo:lc_wfa_w_string_score}. When
...
2$^{40}\approx1.1\times10^{12}$ binary state possibilities; which is
approximately one trillion state possibilities. To provide some context
to this order of magnitude, the aforementioned number of possibilities is
roughly equal to estimated number of stars present in the Andromeda Galaxy
...
\texttt{``[START] 10 day forecast [END]''} is the \texttt{weather/find} label
