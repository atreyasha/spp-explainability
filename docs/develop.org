...
***** SoPa++
****** Lower model
******* update the WFA definitions to signify wildcard transitions which were not present in SoPa
...
******** use omega symbol for wildcards to differentiate from the Kleene star symbol -> think of how to technically motivate this by perhaps allowing omega to be part of the input alphabet -> mention that this is an improvised own definition
****** Upper model  
...
******* show TikZ visualization of computational graph for SoPa++ model
******* update WFA notations where possible
