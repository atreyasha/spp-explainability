...
***** Execution speed
****** investigate exact bottlenack location inside =get_activating_spans= -> particularly in torch apply function -> better to find workaround than use torch for this -> perhaps convert everthing to numpy and operate from there -> perhaps a numpy or base-python equivalent for semirings
******* update data types after changing these such as for utils functions
******* this can then be better parallelized with multipool, use =batch_size= argument for this
****** current model forwards are done by loops, but these can be parallelized or vectorized before =get_activating_spans= and then used inside -> use =batch_size= argument for this
******* compute measure of sparsity -> would be useful as model output diagnostic -> should be roughly half as per how things are done, but can vary significantly by samples
****** port backpointer concept to model as another function which reflects the same workflow
****** use tqdm to help with estimations -> add =tqdm_arg_parser= for posterity and pass around disable tqdm
...
***** Generalizing global explainability
****** perform some kind of clustering to determine which patterns to show/save and which ones not to
****** develop merging framework where regular expressions are generalized from all the best patterns available -> produce pretty and compact ensemble of regular expressions
******* compile regular expressions when doing a search over the mimic model
****** modify =get_nearest_neighbors= to use full embeddings and to use biases as well -> perhaps keep the existing nearest neighbours functionality to deal with similar patterns with different tokens
...
****** find tricks which help to increase generalization such as defaulting to patterns given nearest neighbours or levenstein distance -> or at least discuss them
****** since all transitions are only dependent on the state and not on each other, would it be possible to concatenate all token spans for each pattern and interweave between them?
******* if this is possible, it will hugely benefit global explainability
******* might not be the best solution, but we can experiment how this affects generalization
******* perhaps use graphviz for visualization
***** Quantification and generalizing global explainability
...
****** rename =explain_spp= to something related to globalk explainability and mimic model construction, since another script will be needed to conduct local explanations
...
***** Generic changes
****** re-check token indices in backpointers and ensure they are indeed correct -> do some manual runs and re-thinking
****** think whether to allow transition out of end state and how this works, or what this means and if it is logical
****** look again into document indices and why legacy code subtracted padding tokens
****** look into legacy visualization and interpretation scripts for possible workflows to adopt
...
****** attempt to make normalizer dynamically ignore infinities instead of expecting fixed sizes -> could be done with a simple normalization routine but would have to work on vectoring code
****** consider adding back elementwise affine transformations for LayerNorm -> but this could possibly result in dead patterns to be activated which is an illogical result -> unless we can guarantee that infinity states will always be ignored
...
****** add 2 threads specific arguments to all jarvis shell scripts and commit as local optimizations
