...
***** TODO make workflow to download simple but high-quality NLU dataset and reproduce virtual environment cleanly via poetry
      DEADLINE: <2020-11-01 Sun>
***** TODO either run baseline SoPa on new NLU data or run baseline SoPa on old data to get familiarity
...
***** populate repository with hooks, data downloads, documentation and models -> update metadata in scripts when possible
...
***** *consideration:* use both small and large data sets to get an idea of performance/explainabilityPrepare WMT19, WMT19-AR, WMT16 and PAWS-X data,
