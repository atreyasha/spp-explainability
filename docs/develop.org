...
***** Current
****** TODO make workflow to download Facebook Multilingual Task Oriented Dataset and pre-process to sopa-ready format -> text data and labels with dictionary mapping as to what the labels mean
...
****** TODO major code refactoring for main model (ignore visualization, interpretation and testing) with conversion to recent PyTorch (eg. 1.*) and CUDA versions (eg. 10.*)
       DEADLINE: <2020-12-03 Thu>
******* understand code chunk by chunk in =soft_patterns.py= and =train.py= and replace outdated torch code
******** execute code on low batch level with preset arg namespace -> use this to understand batch level processes and understand limitations
