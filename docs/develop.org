...
******* replace outdated torch code -> no need to declare variables with autograd explicitly: see https://stackoverflow.com/questions/57580202/whats-the-purpose-of-torch-autograd-variable -> replace all Variable calls with simple Tensors and add =requires_grad= argument directly to tensors where this is necessary
******* execute code on low batch level with preset arg namespace in =soft_patterns.py= and =train.py= -> use this to understand batch level processes and limitations

****** work on major changes to architecture
       DEADLINE: <2020-12-24 Thu>
******* initial issues
******** model appears to work out-of-box for multiple classes -> investigate further
...
******** remove sample data set when unnecessary and provide only description of data structures required
