...
**** Methodologies
***** SoPa++
****** Add section describing transparency of SoPa++; specifically that is still a black-box using previous arguments -> make it similar to SoPa section structure and link to following regex proxy section saying this is the post-hoc explainability method
***** Regex proxy
****** SoPa++ simplification to regex proxy
******* describe process of simplification 
...
****** Pseudocode
******* add pseudocode for SoPa++ simplification to get regular expressions
******* add pseudocode for regex proxy compression 
******* report time complexities of the simplification process as well as compression where possible
****** Explain regex proxy forward-pass
******* TODO replace FAs in visualization with regular expression matrix where possible -> perhaps better to add actual regular expressions where possible
******* show TikZ visualization of regex computational graph and tweak colors/labels to make explanations easier if need be
******* make sure to state that the regex's represent substring matches to the borders are generic -> or otherwise explicitly add these to the visualizations -> make this clear both here and for the results to questions three -> alternatively consider different notation to imply this is a regex match with arbitrary borders
****** Explainability
...
***** SoPa vs. SoPa++
****** create table to show exhaustive relevant differences between models, perhaps with a tick and cross to indicate similarities and differences
****** recycle this table later and add it to the SoPa model section
****** list comparisons such as tokenization, input casings, word embeddings, WFAs, output layer, transparencies, explainability techniques and others
...
***** RQ2: Explanations by simplification evaluation
****** mention we are trying to evaluate how close the models are
****** delegate the quality of explainability based on a target audience to future work
...
***** RQ3: Insightful explanations insight
...
**** Results
***** RQ1: Evaluation of test accuracy scores of SoPa++ and regex proxy models
...
***** RQ2: Evaluation of explanations by simplification 
