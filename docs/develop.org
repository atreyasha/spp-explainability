...
** Tasks
*** Current
**** Focus on deriving explainability from trained SoPa++ model(s)
     DEADLINE: <2021-02-14 Sun>
***** Oracle model
****** TODO refactor =visualization= and =interpretation= (two of highest priority) and understand their internal mechanisms
...
***** Considerations post-explainability
...
****** repeat grid-search with multiple random seeds later when explainability process is clearer -> possibly with more parallelized computations to fill up all GPU memory -> would require reading-up on how to do this
****** test workflow for loading pre-computed-patterns to ensure they work without bugs -> missing load information for self-loops, might bug out for case with no self_loops because of index 1 of =diags= and =bias= being updated which is only present with self_loops, perhaps replace with index of -1 -> perhaps can take out numerical epsilon as there are probably some workarounds for this
****** improve cryptic parts of code eg. loading internal model patterns with CW token and numerical epsilon
...
***** create =slurm-s3it= branch later on if it is really necessary
...
***** think more about issues related to using layer normalization of scores -> this could be problematic if all scores are large
...
***** tokenizer configuration should be saved if it is special, nltk tokenizer is probably the same given upstream nltk version
...
***** install torch for cuda 10.1 on upstream servers, ie. =pip install torch==1.7.0+cu101 -f https://download.pytorch.org/whl/torch_stable.html=
**** Documentation
***** consider renaming =soft_patterns_pp= to more elegant term such as =spp=
