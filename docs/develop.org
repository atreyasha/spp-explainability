...
***** TODO start testing SoPa on intent classification data sets to see baseline results -> clone repo and format data to be used directly in soft patterns code base for now
***** TODO recreate virtual environment cleanly, remove boilerplate code and try to create simple models + performance for simple NLU tasks
