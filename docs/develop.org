...
******* think about whether SoPa++ forward pseudocode must be modified with text-level instructions, this might be too complicated with too many details for now -> or maybe change pseudocode to traversal code
...
******* consider changing figure softmax to soft+argmax
...
****** consider adding definitions for main transitions if they are useful
