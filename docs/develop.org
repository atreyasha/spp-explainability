...
***** Current
****** TODO work on major changes to architecture
       DEADLINE: <2020-12-24 Thu>
******* sort out below tasks once more before proceeding
******* execute code on low batch level with preset arg namespace in =soft_patterns.py= and =train.py= -> use this to understand batch level processes and limitations -> correct typing where necessary
******* initial issues
...
******* fine-tuning
******** consider adding mypy as a test case suite -> but use --ignore-missing-imports and --allow-redefinition
...
******** change types where =None= is permitted alongside other type to =Union[Type, None]= -> need to go through all scripts and parse out =None= arguments
