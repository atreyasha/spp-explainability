...
***** Step-by-step
****** TODO major code refactoring for main model (ignore visualization and interpretation) with conversion to recent PyTorch (eg. 1.*) and CUDA versions (eg. 10.*)
       DEADLINE: <2020-11-30 Mon>
******* make arg parser file, preprocessing clean script -> keep things simple for now and lay framework
******* use brute-force method of running code with newer libraries and fix errors on the fly -> replace outdated functions on the fly and make everything clean/modern -> migrate to soft-patterns-pp and clean from there
******* find out whether use_rnn is passed by default and what its purpose generally is -> need to understand all facets of code to master it
******* code works well with latest CUDA and torch, now through code chunk by chunk and execute it to test on data, understand everything and clean errors on the fly -> worry about performance in later steps -> for now focus on understanding everything
...
******* take note that patterns of length one are not allowed, ignore =visualize_efficiently.py= for now
****** choose exact data set and set up workflow to download and pro-process it -> prefer to find a nice benchmark which can be used for extensive comparisons (like RASA NLU)
       DEADLINE: <2020-12-03 Thu>
****** work on changes to architecture
       DEADLINE: <2020-12-24 Thu>
******* check if tokenizer could be replaced from Transformers library for upstream consistency
...
******* dynamic word embeddings and experimenting with more gracious self-loops and epsilon transitions -> perform this incrementally for comparison
...
***** Topic registration: *01.02.21* 
***** Manuscript submission: *18.03.21* 
