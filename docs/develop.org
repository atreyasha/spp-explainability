...
****** change light medium heavy to small medium large and change filenames and log variables as well
...
***** Discussion
****** add statement on quality of explanations based on three criteria in XAI -> link this to RQ2 and find an appropriate place to add it
...
***** remove all neural sopa from readme and everywhere else since spp already implies neural -> leave the arguments in parser the same though
