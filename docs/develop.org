...
********* integrate python logger well with debug argument in legacy sopa -> would be better if everything is done through the logger given logging levels
...
****** provide only description of data structures (eg. data, labels) required for training processes -> remove actual data
