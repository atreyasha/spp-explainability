...
******* find better location to place code from =util.py=
******* make clean preprocessing script with GloVe vectors and explicit name to differentiate from future dynamic sub-word tokenization
******* extract all arg parser chunks and place in dedicated file
******* run code chunk by chunk and remove outdated torch code + understand segments -> file with ~700 sloc 
...
******* find out whether use_rnn is passed by default and what its purpose generally is
...
******* check if tokenizer could be replaced from Transformers library for upstream consistency + add nltk if necessary for punkt tokenization before sentencepiece
...
******* revert back visualization, interpretation and testing scripts into repository for refactoring -> files are currently under local git backup
