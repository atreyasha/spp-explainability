...
****** make separate script for regex compression framework, such as =compress_explainable_spp=
******* develop merging framework where regular expressions are generalized from all the best patterns available
******* provide options such as rational, brave and mad for compression types, indicating how broad they could be
******* look into similar regular expressions across patterns and if this can be optimized somehow
******* produce pretty and compact ensemble of regular expressions
******* perform some kind of clustering to determine which regex patterns to show/save and which ones not to
******* compile regular expressions when doing a search over the mimic model
******* modify =get_nearest_neighbors= to use full embeddings and to use biases as well
******* keep the existing nearest neighbours functionality to deal with similar patterns with different tokens
...
***** Explainability execution speed
****** add batch size to explainability while computing =interim_scores=, but would require that =explain_data= and =explain_text= are sorted beforehand in a symmetric manner and respecting the fact that sorting requires label to be replaced with text, also need to add =batch_size= to =arg_parser= 
****** if applicable, merge efficiencies from =visualize_efficiently= such as using =heapq=
***** Dedicated modelling
...
***** rename =spp_model= to =spp_torch_model= to fit indents better 
***** port backpointer concept to model as another function inside torch model
